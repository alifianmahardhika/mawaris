/**
 * Input ahli waris spesifik satu pihak (keluarga suami atau istri).
 */
...
export function HeirColumnInputs({ side, state, onChange }: Props) {
  ...
  const accent = side === 'suami' ? 'text-blue-700' : 'text-rose-700';
            ...
            <div key={key} className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  disabled={val === 0}
                  className="w-6 h-6 rounded border text-gray-500 hover:bg-gray-100 disabled:opacity-30 text-xs flex items-center justify-center"
                >
                  −
                </button>
                <span className="w-8 text-center text-sm font-medium tabular-nums">{val}</span>
                <button
                  type="button"
